...
const CallBackComponent = () => {
    ...
    const handleCallback = useCallback(async () => {
        ...
        if (status === "failure") {
            handleFailure("Payment Failed", "The payment failed. Please check your bKash account and try again.");
            return;
        }
    ...
    return (
        <div className="fixed inset-0 bg-[#9d9d9c] z-50 flex items-center justify-center">
            <div className="min-h-[300px] flex flex-col items-center justify-center rounded-lg bg-card shadow-2xl border  p-6">
            <div className="text-center flex flex-col items-center justify-center">
                ...
                <h2 className="text-xl font-semibold mb-2 text-white">{isError ? 'Action Required' : 'Processing...'}</h2>
                <p className="text-sm text-black max-w-md text-center">
                {statusMessage}
                </p>
                <p className="text-red-600 font-bold mt-4 text-center">Please do not close or refresh this window until your payment is complete.</p>
            </div>
            </div>
        </div>
    )
